...
if __name__ == "__main__":
    ...
    parser.add_argument('--nargs', required=False, type=int, help="select expression with specific numbre of arguments")
    ...
    if arguments.bench:
        ...
        if not samples_dir.is_dir():
            print("[ERROR] ./samples/ must be a directory")
            sys.exit(1)
        if next(samples_dir.iterdir(), None) != None:
            print("[ERROR] ./samples/ directory must be empty")
            sys.exit(1)
        ...
        for index, expr in enumerate(exprs):
            ...
            if arguments.ite:
                samples = sampleite(expr, exprargs, 100)
            else:
                samples = sample(expr, exprargs, 100)

            json.dump(samples, open("./samples/{}.json".format(index), "w"), indent=4, sort_keys=True)
    elif arguments.expr != None:
        exprargs = getArgs(arguments.expr) 
        samples = sample(arguments.expr, exprargs, 100)
